...
def test_provider_can_update_own_offer(client, db):
    ...
    r = client.patch(f"/api/offers/{offer_id}", headers=auth_header(token), json={
        "title": "Actualizado"
    })
    assert r.status_code == 200
...
def test_provider_cannot_update_other_offer(client, db):
    ...
    r = client.patch(f"/api/offers/{offer_id}", headers=auth_header(token), json={"title": "X"})
    assert r.status_code == 403
...
def test_offer_availability(client, db):
    ...
    # Con disponibilidad
    r = client.get(f"/api/offers/{offer_id}/availability?date=01/11/2025", 
