...
def test_client_list_own_bookings(client, db):
    ...
    client.post("/api/bookings/", headers=ah(token1), json={
        "offer_id": offer_id, "date": "01/11/2025"
    })
    client.post("/api/bookings/", headers=ah(token2), json={
        "offer_id": offer_id, "date": "02/11/2025"
    })
...
def test_client_cannot_confirm_booking(client, db):
    ...
    r2 = client.patch(f"/api/bookings/{booking_id}/status", headers=ah(token), 
