# Cloud Computing: Fundamentos e Infraestructuras - Prácticas

Este es el repositorio de las prácticas de la asignatura de Cloud Computing: Fundamentos e Infraestructuras.

## Descripción del proyecto

Este proyecto consiste en desarrollar una aplicación que permita a la empresa Hamari S.L. gestionar las ventas de sus productos, de forma que permita a los proveedores promocionar sus experiencias de forma sencilla y permita que los clientes los adquieran facilmente. 

<img src="./imgs/logo.png" alt="Logo" style="width:30%;">
(Imagen generado con Claude Sonnet)

## Hito 1
- [Descripción detallada del proyecto](./docs/hito1.md)
    - Objetivos, problemas y tecnologias empleadas 
- [Licencia](./LICENSE)
    - Revisión de los términos y condiciones 

## Hito 2
- [Integración continua](./docs/hito2.md)
    - Descripción de la elección del [gestor de tareas](./docs/hito2/gestor_taras.md)
    - Descripción de la elección de la [biblioteca de aserciones](./docs/hito2/biblioteca_aser.md)
    - Descripción de la elección del [marco de pruebas](./docs/hito2/marco_pruebas.md)
    - Descripción de la elección del [gestor de integración continua](./docs/hito2/gestor_ic.md)
- [Requisitos y uso del repositorio](./docs/requisitos.md)

## Hito 3
- [Diseño de microservicios](./docs/hito3.md)
    - Descripción de la elección del [framework](./docs/hito3/framework.md)

## Hito 4

## Hito 5